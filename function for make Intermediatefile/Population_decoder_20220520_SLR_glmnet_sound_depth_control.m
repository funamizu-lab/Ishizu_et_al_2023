...
----------------------------------------------------------------------------
%}
function Population_decoder_20220520_SLR_glmnet_sound_depth_control(pathname)
...
if max_tif >= 5 %At least 5 neurons for decoding
    ...
    for j = 1:length(selected_window)        %[j,window_length]
        lasso_sound_s(j).matrix = SparseLogisticRegression_CV_20220520_glmnet(all_sound_short(selected_window(j)).matrix',LowHigh_s,SoundEvi_s,Reward_s,block_s,CV_para,CV_repeat);
        lasso_sound_l(j).matrix = SparseLogisticRegression_CV_20220520_glmnet(all_sound_long(selected_window(j)).matrix',LowHigh_l,SoundEvi_l,Reward_l,block_l,CV_para,CV_repeat);
    end
else
    lasso_sound_s = nan;
    lasso_sound_l = nan;
...
save SLR_20220520_glmnet_sound_depth lasso_sound_l lasso_sound_s time_window pre_sound post_sound max_tif selected_window CV_para CV_repeat
