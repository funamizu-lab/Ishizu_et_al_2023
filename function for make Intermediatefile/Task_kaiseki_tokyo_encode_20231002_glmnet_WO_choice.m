...
----------------------------------------------------------------------------
%}
function Task_kaiseki_tokyo_encode_20231002_glmnet_WO_choice(pathname)
...
%Make trial number frame
%trial_number_kernel
for i = 1:length(use_trial)-1
    temp_time1 = state_time(use_trial(i),1).matrix; %Spout start
    temp_time2 = state_time(use_trial(i+1),1).matrix; %Spout start
    
    if i == 1
        trial_number_kernel(1:[temp_time2-1]) = i;
    else
        trial_number_kernel(temp_time1:temp_time2-1) = i;
    end
    
    if i == length(use_trial)-1
        temp3 = end_frame(end) + round(1000/Time_stretch) + 100;
        trial_number_kernel(temp_time2:temp3) = i+1;
    end
end
% figure
% plot(trial_number_kernel)
trial_number_kernel = trial_number_kernel([start_frame:end_frame])';
% figure
% plot(trial_number_kernel)
% hoge
...
% eval(temp_cd);
cd(spike_dir)
mkdir('KernelEncoding_20231002_WO_choice');

%save KernelEncoding_20220320/KernelEncoding_20220320_kernel kernel_all kernel_size_y moto_frame5 CV_para
save KernelEncoding_20231002_WO_choice/KernelEncoding_20231002_kernel_WO_choice kernel_all kernel_size_y CV_para moto_frame_sound
...
%for file_count = 1:max_tif
    ...
    Ridge_neuron = EncodingKernel_Ridge_20220215_glmnet_ver20231002(spike_filter_new, kernel_all,trial_number_kernel, CV_para);
    end    
    save_file = sprintf('KernelEncoding_20231002_WO_choice/KernelEncoding_20231002_glmnet_WO_choice_%d',file_count);
