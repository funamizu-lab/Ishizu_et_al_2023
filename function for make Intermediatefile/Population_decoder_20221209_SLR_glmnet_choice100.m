...
----------------------------------------------------------------------------
%}
function Population_decoder_20221209_SLR_glmnet_choice100(pathname)
...
%each sound make SLR
if length(LeftRight_s) >= use_trial && max_tif >= use_neuron
%if max_tif >= use_neuron
    ...
    if length(test0) < 2 ||  length(test1) < 2
        ...
        for j = 1:length(selected_window)
            %[j,window_length]
            %[short_correct_rate(:,j),short_ave_likeli(:,j)] = SparseLogisticRegression_CV_20220404_glmnet_choice2(all_sound_short(selected_window(j)).matrix,LeftRight_s,LeftRight_s,SoundEvi_s,Reward_s,block_s,CV_para,CV_repeat,use_neuron,use_trial);
            [short_correct_rate(:,j),short_ave_likeli(:,j),short_dp(:,j)] = SparseLogisticRegression_CV_20221201_glmnet_choice2(all_sound_short(selected_window(j)).matrix,LeftRight_s,LeftRight_s,SoundEvi_s,Reward_s,block_s,CV_para,CV_repeat,use_neuron,use_trial);
        end
    end
else
        short_correct_rate = [];
...
if length(LeftRight_l) >= use_trial && max_tif >= use_neuron
%if max_tif >= use_neuron
    ...
    if length(test0) < 2 ||  length(test1) < 2
        ...
        for j = 1:length(selected_window)
            %[j,window_length]
            %[long_correct_rate(:,j),long_ave_likeli(:,j)] = SparseLogisticRegression_CV_20220404_glmnet_choice2(all_sound_long(selected_window(j)).matrix,LeftRight_l,LeftRight_l,SoundEvi_l,Reward_l,block_l,CV_para,CV_repeat,use_neuron,use_trial);
            [long_correct_rate(:,j),long_ave_likeli(:,j),long_dp(:,j)] = SparseLogisticRegression_CV_20221201_glmnet_choice2(all_sound_long(selected_window(j)).matrix,LeftRight_l,LeftRight_l,SoundEvi_l,Reward_l,block_l,CV_para,CV_repeat,use_neuron,use_trial);
        end
    end
else
        long_correct_rate = [];
...
save SLR100_20221209_glmnet_choice short_correct_rate short_ave_likeli long_correct_rate long_ave_likeli use_neuron use_trial length_trial selected_window long_dp short_dp
