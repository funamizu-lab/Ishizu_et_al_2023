...
----------------------------------------------------------------------------
%}
function Population_decoder_20220912_SLR_glmnet_prior100(pathname)
...
%if max_tif >= use_neuron
    ...
    test0 = find(binary_prior_short23(1:use_trial) == 0);
    test1 = find(binary_prior_short23(1:use_trial) == 1);
    if length(test0) < 2 ||  length(test1) < 2
        ...
        for j = 1:length(selected_window)
            %[j,window_length]
            [short_correct_rate(:,j),short_ave_likeli(:,j)] = SparseLogisticRegression_CV_20220404_glmnet_sound2(all_sound_short(selected_window(j)).matrix,binary_prior_short23,LeftRight_s,SoundEvi_s,Reward_s,block_s,CV_para,CV_repeat,use_neuron,use_trial);
        end
    end
else
        short_correct_rate = [];
...
%if max_tif >= use_neuron
    test0 = find(binary_prior_long23(1:use_trial) == 0);
    test1 = find(binary_prior_long23(1:use_trial) == 1);
    if length(test0) < 2 ||  length(test1) < 2
        long_correct_rate = [];
        long_ave_likeli = [];
    else
        ...
        for j = 1:length(selected_window)
            %[j,window_length]
            [long_correct_rate(:,j),long_ave_likeli(:,j)] = SparseLogisticRegression_CV_20220404_glmnet_sound2(all_sound_long(selected_window(j)).matrix,binary_prior_long23,LeftRight_l,SoundEvi_l,Reward_l,block_l,CV_para,CV_repeat,use_neuron,use_trial);
        end
    end
else
        long_correct_rate = [];
...
save SLR100_20220912_glmnet_prior short_correct_rate short_ave_likeli long_correct_rate long_ave_likeli use_neuron use_trial length_trial selected_window
