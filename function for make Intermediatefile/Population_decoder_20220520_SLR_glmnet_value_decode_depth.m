...
----------------------------------------------------------------------------
%}
function Population_decoder_20220520_SLR_glmnet_value_decode_depth(pathname)
...
temp = dir('RL_20220314*'); %RL_20220118 frame
if length(temp) ~= 1
    temp
    hoge
end
...
[prior,posterior,ave_likeli,likelihood,Long_posterior,Short_posterior,...
    ...
    Dual_RL_model_block1_20220314_para_determined(Bpod_file,para_max(2,:),N_trial);
...
%Binary prior
[binary_prior,binary_prior_long23,binary_prior_short23] = get_binary_prior(prior_value,use_trial,long23_prior,short23_prior);
%Binary long value
[binary_longV,binary_longV_long23,binary_longV_short23] = get_binary_prior(long_value,use_trial,long23_prior,short23_prior);
%Binary short value
[binary_shortV,binary_shortV_long23,binary_shortV_short23] = get_binary_prior(short_value,use_trial,long23_prior,short23_prior);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
CV_repeat = 100;
CV_para = 10;
%It is too redundant but make all decoders with 3 values are possible
%each sound make SLR

if max_tif >= 5 %At least 5 neurons for decoding
    test0 = find(binary_prior == 0);
    test1 = find(binary_prior == 1);
    [length(test0),length(test1)]
    if length(test0) < 2 ||  length(test1) < 2
        lasso_sound_s = nan;
        lasso_sound_l = nan;
        lasso_sound_l_short = nan;
    else
        %for j = 1:window_length
        for j = 1:length(selected_window)
            %[j,window_length]
            lasso_sound_s(j).matrix = SparseLogisticRegression_CV_20220520_glmnet(all_sound_short(selected_window(j)).matrix',binary_shortV_short23,SoundEvi_s,Reward_s,block_s,CV_para,CV_repeat);
            lasso_sound_l(j).matrix = SparseLogisticRegression_CV_20220520_glmnet(all_sound_long(selected_window(j)).matrix',binary_longV_long23,SoundEvi_l,Reward_l,block_l,CV_para,CV_repeat);
            lasso_sound_l_short(j).matrix = SparseLogisticRegression_CV_20220520_glmnet(all_sound_long(selected_window(j)).matrix',binary_shortV_long23,SoundEvi_l,Reward_l,block_l,CV_para,CV_repeat);
        end
    end
else
    lasso_sound_s = nan;
...
save SLR_20220520_glmnet_value_para2_depth lasso_sound_l lasso_sound_l_short lasso_sound_s time_window pre_sound post_sound max_tif selected_window CV_para CV_repeat

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [binary_prior,binary_prior_long23,binary_prior_short23] = get_binary_prior(prior_value,use_trial,long23_prior,short23_prior)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
