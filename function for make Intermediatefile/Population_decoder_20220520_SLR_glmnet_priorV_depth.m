...
----------------------------------------------------------------------------
%}
function Population_decoder_20220520_SLR_glmnet_priorV_depth(pathname)
...
if max_tif >= 5 %At least 5 neurons for decoding
    ...
    if length(test0) < 2 ||  length(test1) < 2
        ...
        for j = 1:length(selected_window)
            %[j,window_length]
            lasso_sound_s(j).matrix = SparseLogisticRegression_CV_20220520_glmnet(all_sound_short(selected_window(j)).matrix',binary_prior_short23,SoundEvi_s,Reward_s,block_s,CV_para,CV_repeat);
            lasso_sound_l(j).matrix = SparseLogisticRegression_CV_20220520_glmnet(all_sound_long(selected_window(j)).matrix',binary_prior_long23,SoundEvi_l,Reward_l,block_l,CV_para,CV_repeat);
        end
    end
else
    lasso_sound_s = nan;
...
save SLR_20220520_glmnet_priorV_para2_depth lasso_sound_l lasso_sound_s time_window pre_sound post_sound max_tif selected_window CV_para CV_repeat
