...
----------------------------------------------------------------------------
%}
function norm_Task_kaiseki_tokyo1_20220316_sound_choice_separate2_anova(pathname)
...
[long1_p_anovan,long1_p_choice0,long1_p_choice1,long1_mean_choice0,long1_mean_choice1,long1_mean,...
 long2_p_anovan,long2_p_choice0,long2_p_choice1,long2_mean_choice0,long2_mean_choice1,long2_mean] = ...
...
[short1_p_anovan,short1_p_choice0,short1_p_choice1,short1_median_choice0,short1_median_choice1,short1_median,...
 short2_p_anovan,short2_p_choice0,short2_p_choice1,short2_median_choice0,short2_median_choice1,short2_median] = ...
...
save norm_Tokyo2_detail_20220316_anova ...
    long1_p_anovan long1_p_choice0 long1_p_choice1 long1_mean_choice0 long1_mean_choice1 long1_mean ...
    long2_p_anovan long2_p_choice0 long2_p_choice1 long2_mean_choice0 long2_mean_choice1 long2_mean ...
    short1_p_anovan short1_p_choice0 short1_p_choice1 short1_median_choice0 short1_median_choice1 short1_median ...
    short2_p_anovan short2_p_choice0 short2_p_choice1 short2_median_choice0 short2_median_choice1 short2_median
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [stim1_p_anovan,stim1_p_choice0,stim1_p_choice1,stim1_mean_choice0,stim1_mean_choice1,stim1_mean,...
 stim2_p_anovan,stim2_p_choice0,stim2_p_choice1,stim2_mean_choice0,stim2_mean_choice1,stim2_mean] = ...
    get_task_activ_separate_detail(use_long,frame_sound,frame_choice_select,frame_spout,max_tif,...
    time_long_pre,time_long_post,time_long_pre2,time_long_post2,time_window,time_window_base, ...
...
[stim1_p_anovan,stim1_p_choice0,stim1_p_choice1,stim1_mean_choice0,stim1_mean_choice1,stim1_mean] = ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [p_anovan,p_choice0,p_choice1,mean_choice0,mean_choice1,mean_activ] = ...
    get_basic_activity(max_tif, time_long, frame_sound_use_on, tone_evidence, frame_sound, use_long,time_window,...
    trial_H_correct,trial_L_correct,trial_H_error,trial_L_error,high,low,right,left,prior1,prior0, ...
    evi_stim,evi_correct,evi_prior1,evi_prior0,evi_correct_prior1,evi_correct_prior0)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%for file_count = 1:max_tif
    ...
    index_choice = [];
    for k = 1:6
        temp1 = evi_correct_prior1(k).matrix;
        temp0 = evi_correct_prior0(k).matrix;
        index_prior = [index_prior; ones(length(temp1),1); zeros(length(temp0),1)];
        if k <= 3
            index_choice = [index_choice; zeros(length(temp1),1); zeros(length(temp0),1)];
        else
            index_choice = [index_choice; ones(length(temp1),1); ones(length(temp0),1)];
        end
    end
    temp_c0 = find(index_choice == 0);
    temp_c1 = find(index_choice == 1);
    temp_p0 = find(index_prior == 0);
    temp_p1 = find(index_prior == 1);
    temp_c0p0 = intersect(temp_c0, temp_p0);
    temp_c0p1 = intersect(temp_c0, temp_p1);
    temp_c1p0 = intersect(temp_c1, temp_p0);
    temp_c1p1 = intersect(temp_c1, temp_p1);
...
%             stim1_correct_prior0(file_count,k,j) = mean(spike_frame1(evi_correct_prior0(k).matrix,j));
            temp_activ = [temp_activ; spike_frame1(evi_correct_prior1(k).matrix,j); spike_frame1(evi_correct_prior0(k).matrix,j)];
            
            mean_activ1(k) = mean(spike_frame1(evi_correct_prior1(k).matrix,j));
            mean_activ0(k) = mean(spike_frame1(evi_correct_prior0(k).matrix,j));
        end
        temp_p = anovan(temp_activ,{index_prior,index_choice},'display','off');
        
        p_anovan(file_count).matrix(j,:) = temp_p';
        %in each choice
        p_choice0(file_count).matrix(j,1) = ranksum(temp_activ(temp_c0p0),temp_activ(temp_c0p1));
        p_choice1(file_count).matrix(j,1) = ranksum(temp_activ(temp_c1p0),temp_activ(temp_c1p1));
        
        mean_activ(file_count).matrix(j,:) = [mean(mean_activ1),mean(mean_activ0)];
        mean_choice0(file_count).matrix(j,:) = [mean(mean_activ1(1:3)),mean(mean_activ0(1:3))];
        mean_choice1(file_count).matrix(j,:) = [mean(mean_activ1(4:6)),mean(mean_activ0(4:6))];
    end
%     p_anovan(file_count).matrix = temp_p_anovan;
%     p_choice0(file_count).matrix = temp_p_choice0;
%     p_choice1(file_count).matrix = temp_p_choice1;
end
