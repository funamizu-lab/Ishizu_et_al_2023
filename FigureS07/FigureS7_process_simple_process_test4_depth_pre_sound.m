...
----------------------------------------------------------------------------
%}
function FigureS7_process_simple_process_test4_depth_pre_sound(folders)
...
writetable(T, ['source fig ',name,' S7 righttoppannel left.csv']);

subplot(1,3,2)
sdata = plot_prior_block_trace(short_frame,evi_prefer_all,evi_nonprefer_all,temp_x);
%%% source data %%%
T = struct2table(sdata);
writetable(T, ['source fig ',name,' S7 righttoppannel mid.csv']);

subplot(1,3,3)
sdata = plot_prior_block_trace(long_frame,evi_prefer_all,evi_nonprefer_all,temp_x);
%%% source data %%%
T = struct2table(sdata);
writetable(T, ['source fig ',name,' S7 righttoppannel right.csv']);
...
%%% source data %%%
T = struct2table(sdata);
writetable(T, ['source fig ',name,' S7 midlle top.csv']);
