...
sdata1.time = sort_sabun;

figure
imagesc(sig_sort)
% make_red_blue_colormap_20230710(sig_sort,color_step_size)
sdata2.time = sig_sort;

cd('G:\upload_code\FigureS5\FigS5c');
name = extractBefore(folders,'_');
T = struct2table(sdata1);
writetable(T, ['source fig ',name,' S5c leftpannel heatmap left.csv']);
T = struct2table(sdata2);
writetable(T, ['source fig ',name,' S5c leftpannel heatmap right.csv']);
