...
cd('G:\upload_code\FigureS5\FigS5a');
figure
subplot(1,3,1); hold on
plot_correct_error(short_correct_all,sig_correct_all,17,name);
...
sdata.x = short_correct_all(17).matrix;
sdata.y = sig_correct25_all17;
T = struct2table(sdata);
writetable(T, ['source figS5a middle ',name,'.csv']);
...
sdata = struct();% source data 
sdata.x = sabun_sound_initial;
sdata.y = sabun_sound_end;
T = struct2table(sdata);
writetable(T, ['source figS5a right ',name,'.csv']);

%% Fig S5b left %Before sound, compare the prior and choice index %%%
cd('G:\upload_code\FigureS5\FigS5b');
figure
abs_plot_prior_choice(sig_prior_all,sig_choice_all,15,name);

%%% Fig S5b right %%%
figure; hold on
plot_ROC(ROC_prior_all,ROC_choice_all,15,name);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function plot_ROC(sig_sound_all,sig_choice_all,use_frame,name)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
sdata = struct();% source data 
sdata.x = sig_sound_all(use_frame).matrix;
sdata.y = sig_choice_all(use_frame).matrix;
T = struct2table(sdata);
writetable(T, ['source figS5b right ',name,'.csv']);
return

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function abs_plot_prior_choice(sig_error_all,sig_correct_all,use_frame,name)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
sdata.y = temp2;
T = struct2table(sdata);
writetable(T, ['source figS5b left ',name,'.csv']);
return


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function plot_correct_error(sig_error_all,sig_correct_all,use_frame,name)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
sdata = struct();% source data 
sdata.x = sig_error_all(use_frame).matrix;
sdata.y = sig_correct_all(use_frame).matrix;
T = struct2table(sdata);
writetable(T, ['source figS5a left ',name,'.csv']);
return