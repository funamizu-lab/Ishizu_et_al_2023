...
sdata.choice = temp1_WO_conf(:,1);
sdata.prior = temp1_WO_conf(:,2);
sdata.choice_prior = temp1_WO_conf(:,3);
T = struct2table(sdata);
writetable(T, ['source figS8b choice left ',name,'.csv']);

ax(2) = subplot(1,3,2);
boxplot(ax(2), temp2_WO_conf(:,1:3), 'symbol', '')
sdata = struct();% source data
sdata.choice = temp2_WO_conf(:,1);
sdata.prior = temp2_WO_conf(:,2);
sdata.choice_prior = temp2_WO_conf(:,3);
T = struct2table(sdata);
writetable(T, ['source figS8b choice mid ',name,'.csv']);

ax(3) = subplot(1,3,3);
boxplot(ax(3), temp3_WO_conf, 'symbol', '')
sdata = struct();% source data
sdata.choice = temp3_WO_conf(:,1);
sdata.prior = temp3_WO_conf(:,2);
sdata.choice_prior= temp3_WO_conf(:,3);
