...
----------------------------------------------------------------------------
%}
function FigureS8b_process_20231223_sound(folders)

analysis_dir = eval(folders);
use_para = [5,3,10,4]; %for sound
...
%%% Figure S8b for ac %%%
cd('G:\upload_code\FigureS8\FigS8b');
...
sdata.sound = temp1_WO_conf(:,1);
sdata.prior = temp1_WO_conf(:,2);
sdata.sound_prior = temp1_WO_conf(:,3);
T = struct2table(sdata);
writetable(T, ['source figS8b sound left ',name,'.csv']);

ax(2) = subplot(1,3,2);
boxplot(ax(2), temp2_WO_conf(:,1:3), 'symbol', '')
sdata = struct();% source data
sdata.sound = temp2_WO_conf(:,1);
sdata.prior = temp2_WO_conf(:,2);
sdata.sound_prior = temp2_WO_conf(:,3);
T = struct2table(sdata);
writetable(T, ['source figS8b sound mid ',name,'.csv']);

ax(3) = subplot(1,3,3);
boxplot(ax(3), temp3_WO_conf, 'symbol', '')
sdata = struct();% source data
sdata.sound = temp3_WO_conf(:,1);
sdata.prior = temp3_WO_conf(:,2);
sdata.sound_prior= temp3_WO_conf(:,3);
sdata.actionvalue = temp3_WO_conf(:,4);
T = struct2table(sdata);
writetable(T, ['source figS8b choice right ',name,'.csv']);
