...
----------------------------------------------------------------------------
%}
function Figure3d_process_20230701_simple_process3_raw_depth_control(folders)
...
cd('G:\upload_code\Figure3\Fig3c');

tmp = dir('fig3c*.mat');
load(tmp.name);

cd('G:\upload_code\Figure3\Fig3d');
figure
subplot(2,2,1)
...
writetable(T, 'source fig3d left top.csv');
...
sdata.toneindex_correct= recval(:,2);
T = struct2table(sdata);
writetable(T, ['source fig3d right top.csv']);
...
T = struct2table(sdata);
writetable(T, ['source fig3d left bottom.csv']);
...
sdata.ROC_choice= recval(:,2);
T = struct2table(sdata);
writetable(T, ['source fig3d right bottom.csv']);
