...
----------------------------------------------------------------------------
%}
function FigureS10_20230923_simple_process_test5_depth_integrate_CV2
...
close all
%%% Fig S10 raw data %%%
cd('G:\upload_code\FigureS10');
figure
sdata = struct(); % source data
sdata.x = (1:length(use_frame))'; 
subplot(2,2,2)
[mean_trace,~,se_trace] = plot_median_se_moto_x_axis_matrix(mpfc_choice, [1:length(use_frame)], [0 0 0],2);
sdata.topleft_mpfc_mean = mean_trace';
sdata.topleft_mpfc_se = se_trace';
subplot(2,2,4)
[mean_trace,~,se_trace] = plot_median_se_moto_x_axis_matrix(fof_choice, [1:length(use_frame)], [0 0 0],2);
sdata.topleft_m2_mean = mean_trace';
sdata.topleft_m2_se = se_trace';

subplot(2,2,1)
[mean_trace,~,se_trace] = plot_median_se_moto_x_axis_matrix(mpfc_prior, [1:length(use_frame)], [0 0 0],2);
sdata.bottomleft_mpfc_mean = mean_trace';
sdata.bottomleft_mpfc_se = se_trace';
subplot(2,2,3)
[mean_trace,~,se_trace] = plot_median_se_moto_x_axis_matrix(fof_prior, [1:length(use_frame)], [0 0 0],2);
sdata.bottomleft_m2_mean = mean_trace';
sdata.bottomleft_m2_se = se_trace';

%%% source data %%%
T = struct2table(sdata);
writetable(T, 'source fig S10 rawdata.csv');
...
%%% Fig S10 model fitting %%%
CV_para = 10;

disp('prior all')% Fig S10 bottom left 
sabun_all=CV_computation_mpfc_fof(mpfc_prior, fof_prior, [1,length_data], CV_para);

disp('prior without first')% Fig S10 bottom right
sabun_all=CV_computation_mpfc_fof(mpfc_prior, fof_prior, [2,length_data], CV_para);

disp('choice all')% Fig S10 top left 
sabun_all=CV_computation_mpfc_fof(mpfc_choice, fof_choice, [1,length_data], CV_para);

disp('choice without first')% Fig S10 top right
sabun_all=CV_computation_mpfc_fof(mpfc_choice, fof_choice, [2,length_data], CV_para);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function sabun_all=CV_computation_mpfc_fof(mpfc_prior, fof_prior, use_data, CV_para)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
