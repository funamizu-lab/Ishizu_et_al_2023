...
----------------------------------------------------------------------------
%}
function Figure6e_process_20220520_0912_SLR_long_detail_limit_neuron2(folders)
...
writetable(T, ['source fig6e right ',name,'.csv']);
...
if(ismember({'auc'},name))
    ...
    T = struct2table(sdata);
    writetable(T, ['source fig6e middle ',name,'.csv']);
end
