function FigureS11a_drawEachNeuronTrace_TCchoice
...
for j=1:6
    if(flip_label(i)==0)
        y=evi_correct_ALL{i,j}*1000;
        plot(y,'Color',col(j,:));
        eval(['sdata.',tag{end-j+1},'=transpose(y)']);
    else
        y=evi_correct_ALL{i,6-j+1}*1000;
        plot(y,'Color',col(j,:));
        eval(['sdata.',tag{end-j+1},'=transpose(y)']);
    end
end
...
ylabel('Spike (Hz)');
sdata.xtime_mscec=(1:length(y))'-sound_pre;
%%% source data %%%
T = struct2table(sdata);
writetable(T, 'source fig S11a left.csv');

sdata = struct();% source data 
subplot(1,3,2); hold on;
for j=1:6
    if(flip_label(i)==0)
        y=evi_error_ALL{i,j}*1000;
        plot(y,'Color',col(j,:));
        eval(['sdata.',tag{end-j+1},'=transpose(y)']);
    else
        y=evi_error_ALL{i,6-j+1}*1000;
        plot(y,'Color',col(j,:));
        eval(['sdata.',tag{end-j+1},'=transpose(y)']);
    end
end
...
%%% source data %%%
T = struct2table(sdata);
writetable(T, 'source fig S11a middle.csv');
...
sdata.x=x';
sdata.correct=corre';
sdata.correct_se=c_se';
sdata.error=error';
sdata.error_se=e_se';

T = struct2table(sdata);
writetable(T, 'source fig S11a right.csv');
end
