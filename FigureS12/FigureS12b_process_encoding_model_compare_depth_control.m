...
function FigureS12b_process_encoding_model_compare_depth_control

[auc_sound,auc_choice,auc_prior, auc_prob, auc_neuron] = process_20231002_encoding_model2_depth('auc_20230907_n');
[fof_sound,fof_choice,fof_prior, fof_prob, fof_neuron] = process_20231002_encoding_model2_depth('fof_20230907_n');
[mpfc_sound,mpfc_choice,mpfc_prior, mpfc_prob, mpfc_neuron] = process_20231002_encoding_model2_depth('mpfc_20230907_n');
close all
...
%%%%%%%%
% all recorded neurons of each region
auc_all_neuron = 5994;
fof_all_neuron = 3854;
mpfc_all_neuron = 9503;
%%%%%%%%

%%% Fig S12b left %%%
cd('G:\upload_code\FigureS12\FigS12b');
figure;
sdata=struct();
sdata.label={'sound';'choice';'prior'};
bar([auc_prob*auc_neuron(4)/auc_all_neuron;...
    fof_prob*fof_neuron(4)/fof_all_neuron;...
    mpfc_prob*mpfc_neuron(4)/mpfc_all_neuron]')
auc_neuron_n = [auc_neuron(1:3), auc_all_neuron];
fof_neuron_n = [fof_neuron(1:3), fof_all_neuron];
mpfc_neuron_n = [mpfc_neuron(1:3), mpfc_all_neuron];

%%% source data %%%
sdata.barvalue_ac =(auc_prob*auc_neuron(4)/auc_all_neuron)';
sdata.barvalue_m2 =(fof_prob*fof_neuron(4)/fof_all_neuron)';
sdata.barvalue_mpfc =(mpfc_prob*mpfc_neuron(4)/mpfc_all_neuron)';
T = struct2table(sdata);
writetable(T, 'source fig S12b left.csv');

p_sound_all = kai2test_3regions(auc_neuron_n,fof_neuron_n,mpfc_neuron_n,1);
p_choice_all = kai2test_3regions(auc_neuron_n,fof_neuron_n,mpfc_neuron_n,2);
p_prior_all = kai2test_3regions(auc_neuron_n,fof_neuron_n,mpfc_neuron_n,3);

p_sound_all
p_choice_all
p_prior_all

[auc_prob*auc_neuron(4)/auc_all_neuron;...
    fof_prob*fof_neuron(4)/fof_all_neuron;...
    mpfc_prob*mpfc_neuron(4)/mpfc_all_neuron]
...
figure
sdata=struct();
bar(1, [auc_neuron(4)/auc_all_neuron,...
    fof_neuron(4)/fof_all_neuron,...
    mpfc_neuron(4)/mpfc_all_neuron])
use_auc = [auc_neuron(4), auc_all_neuron - auc_neuron(4)];
use_fof = [fof_neuron(4), fof_all_neuron - fof_neuron(4)];
use_mpfc = [mpfc_neuron(4), mpfc_all_neuron - mpfc_neuron(4)];
disp('proportion of task relevant neuron')
[auc_neuron(4)/auc_all_neuron fof_neuron(4)/fof_all_neuron mpfc_neuron(4)/mpfc_all_neuron]
[kai2_test(use_auc,use_fof); kai2_test(use_auc,use_mpfc); kai2_test(use_mpfc,use_fof)]

%%% source data %%%
sdata.barvalue_ac =auc_neuron(4)/auc_all_neuron;
sdata.barvalue_m2 =fof_neuron(4)/fof_all_neuron;
sdata.barvalue_mpfc =mpfc_neuron(4)/mpfc_all_neuron;
T = struct2table(sdata);
writetable(T, 'source fig S12b right.csv');
