...
function B = Figure1e_Dual_behave_analysis_block1_220818_lick_long4
...
for i = 1:length(analysis_folder)
    ...
    for filecount = 1 : length(temp_file)
        ...
        [temp_B_max, temp_B_prior_max, stats_max, temp_B_min, temp_B_prior_min, stats_min] = ...
            behave_analysis_block1_201028_lick_each(fpath, time_bin);
        close all
...
plot_B_prior2(B_prior,x_bin,all_subject);

% save lme4_behave_analysis_block1_220818_lick_long lme_each_sound lme_prior p p_each p_prior time_bin p_sound_subject p_prior_subject
...
figure
sdata = struct();
[mean_trace,~,se_trace] = plot_mean_se_moto(B_prior(:,2:size_B-1),[0 0 0],2);% mean + se
set(gca,'xlim',x_bin)

sdata.mean=mean_trace';
sdata.se =se_trace';
T = struct2table(sdata);
writetable(T, 'source fig1e long.csv');

% p = kruskalwallis(B_prior(:,2:size_B-1),[],'off');
% 
% for i = 1:max(all_subject)
% %     temp_subject = find(all_subject == i);
...
max_trial = intersect(use_trial_all, maxD_trial);

[B_max, B_prior_max, stats_max] = ...
...
% temp = find(Correct_trial == 1); %Right is correct
if mean(binary_all_tone(Correct_trial == 1)) < 0.5 %low for right correct
    disp('flip tones')
%     binary_all_tone = -binary_all_tone;
