...
----------------------------------------------------------------------------
%}
function Figure4a_process_20230711_simple_process_test4_depth_control(folders)
...
%% fig f left
cd('G:\upload_code\Figure4\Fig4a');
use_color = jet(6);
figure; hold on
...
writetable(T, 'source fig4a left.csv');
...
T = struct2table(sdata);
writetable(T, 'source fig4a right top.csv');
...
T = struct2table(sdata);
writetable(T, 'source fig4a right bottom.csv');
