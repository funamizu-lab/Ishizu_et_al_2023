...
----------------------------------------------------------------------------
%}
function Figure_i_process_20230704_simple_process_test4_depth_control(folders)
...
%% figure i
cd('G:\upload_code\Figure3_4_S8\i');
region_num={'3','4','S8'};
...
for i = 1:3
    subplot(1,3,i)
    hold on
    [mean_trace,~,se_trace]= plot_median_se_each_nan(L_prior(i).matrix,hist_count,[0 0 0],2);
    eval(['sdata.',tag{i},'_priorval_median=transpose(mean_trace)']);
    eval(['sdata.',tag{i},'_priorval_se=transpose(se_trace)']);
    [mean_trace,~,se_trace]= plot_median_se_each_nan(L_conf(i).matrix,hist_count,[0 154 205]./255,2);
    eval(['sdata.',tag{i},'_actionval_median=transpose(mean_trace)']);
