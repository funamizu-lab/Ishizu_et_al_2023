...
sdata.x = plot_history';
sdata.totaldata_mean = mean(sabun_new_likeli)';
sdata.totaldata_se = std(sabun_new_likeli)' ./ (sqrt(size(sabun_new_likeli,1)));
for i = 1:max_mouse
eval(['sdata.mouse',num2str(i),'_mean = transpose(sabun_new_mouse_likeli(i,:));']);
end

cd('G:\upload_code\FigureS1\FigS1c');

T = struct2table(sdata);
writetable(T, 'source figS1c.csv');
