...
function FigureS1a_Dual_Full_psycho_analysis_230630_all_session
...
for i = 1:length(analysis_folder)
    ...
    %%% Fig S1a %%%
    figure
    cd('G:\upload_code\FigureS1\FigS1a');
    ...
    sdata1.x=evi_x';
    sdata1.session = opt_min_L';
    T = struct2table(sdata1);
    writetable(T, ['mouse', num2str(i),' source figS1a short leftblock.csv']);
    sdata2.x=evi_x';
    sdata2.session = opt_min_R';
    T = struct2table(sdata2);
    writetable(T, ['mouse', num2str(i),' source figS1a short rightblock.csv']);
    ...
    sdata1.session = opt_min_L';
    T = struct2table(sdata1);
    writetable(T, ['mouse', num2str(i),' source figS1a long leftblock.csv']);
    sdata2.x=evi_x';
    sdata2.session = opt_min_R';
    T = struct2table(sdata2);
    writetable(T, ['mouse', num2str(i),' source figS1a long rightblock.csv']);
    ...
    sdata.x=evi_x';
    sdata.short_left=mouse_min_L(i,:)';
    sdata.short_right=mouse_min_R(i,:)';
    sdata.long_left=mouse_max_L(i,:)';
    sdata.long_right=mouse_max_R(i,:)';
    T = struct2table(sdata);
    writetable(T, ['mouse', num2str(i),' source figS1a grand average.csv']);
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function plot_psychometric_session(evi_x,min_L,min_R,sound_evi,conf_min_L,conf_min_R, r_L, r_R)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mean_r_L = mean(r_L);
mean_r_R = mean(r_R);
std_r_L = std(r_L);
...
[log_likelli,opt,X] = Opt_psychometric_Gauss_max(X, Chosen_side, trial_evidence, evi_x, lapse);
...
[N_trial,size_x] = size(tone_evi);
likelihood = zeros(1,N_trial);

%Trial by trial, get the likelihood
clear temp_p temp_exp
%temp_right = normcdf(para(1), tone_evi, para(2), 'upper') - normcdf(1, tone_evi, para(2), 'upper'); %Right choice probablity
