function FigureS3d_NeuralSequenceTest_NormRescaleFilter

parent='G:/Ishizu_data';
dataPath='/Revise_ishizu/output/SpikeMark_longtone';
outPath='/Revise_ishizu/output/NeuralSequenceTest3';
...
%     test_data2=zscore(test2(:,soundPeriod),0,2);
    ...
    if(i==1)
        ...
        %%% source data %%%
        datalabel = extractBefore(figname1,' ');
        sdata = struct();% source data
        sdata.time = g_data;
        T = struct2table(sdata);
        writetable(T, ['source figS3d top left heatmap ',datalabel,'.csv']);
...
%%% source data %%%
sdata = struct();% source data
sdata.neuron_id = y_sound';
sdata.peaktime_msec = (mean(testdata2)-100)';
sdata.peaktime_sd_msec = (std(testdata2))';
T = struct2table(sdata);
writetable(T, ['source figS3d top right ',datalabel,'.csv']);
...
for i=1:length(neuron)
    ...
    trialNum= size(neuron(i).norm_spike_filter,1);% use trial length
    end
end
...
for i=1:length(sigid)
    spike_trace= neuron(sigid(i)).norm_spike_filter;
