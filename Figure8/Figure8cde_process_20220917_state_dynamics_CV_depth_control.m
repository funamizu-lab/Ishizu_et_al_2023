...
----------------------------------------------------------------------------
%}
function Figure8cde_process_20220917_state_dynamics_CV_depth_control(folders)
...
%% Fig 8d 
cd('G:\upload_code\Figure8\Fig8cde');
...
writetable(T, ['source fig 8d_',name,'.csv']);
...
T = struct2table(sdata);
writetable(T, ['source fig 8e left_',name,'.csv']);
...
T = struct2table(sdata);
writetable(T, ['source fig8c ',name,'.csv']);
end
...
T = struct2table(sdata);
writetable(T, ['source fig 8e right top_',name,'.csv']);
...
T = struct2table(sdata);
writetable(T, ['source fig 8e right bottom_',name,'.csv']);
return
