...
----------------------------------------------------------------------------
%}
function FigureS14ab_process_SLR_prior_block_changes
...
%%% FigureS14b left %%%
cd('G:\upload_code\FigureS14\FigS14b');
auc_b(auc_b(:,2) < 0.004,:) = [];
fof_b(fof_b(:,2) < 0.004,:) = [];
...
writetable(T, 'source figS14b left ac.csv');

sdata = struct();
sdata.x_m2 = fof_b(:,2);
sdata.y_m2 = fof_b(:,1);
T = struct2table(sdata);
writetable(T, 'source figS14b left m2.csv');

sdata = struct();
sdata.x_mpfc = mpfc_b(:,2);
sdata.y_mpfc = mpfc_b(:,1);
T = struct2table(sdata);
writetable(T, 'source figS14b left mpfc.csv');
...
sdata.label= group;
T = struct2table(sdata);
writetable(T, 'source figS14b right.csv');
...
sdata = norm_parameter(ave_block_change_prior, ave_RL_prior, 0);
% norm_parameter(ave_block_change_prior, ave_RL_prior, 4);
% norm_parameter(ave_block_change_prior, ave_RL_prior, 9);

%%% source data %%%
cd('G:\upload_code\FigureS14\FigS14a');
name = extractBefore(folders,'_');
T = struct2table(sdata);
writetable(T, ['source figS14a ',name,'.csv']);

return


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [sdata,prior_change,RL_change,session_not_use] = norm_parameter(ave_block_change_prior, ave_RL_prior, last_trial)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[mean_trace,~,se_trace] = plot_mean_se_moto(norm_prior,[0 0 1],2);
sdata.x= (1:length(mean_trace))'-21;
sdata.decorded=mean_trace';
sdata.decorded_se=se_trace';

% sdata = 
[mean_trace,~,se_trace] = plot_mean_se_moto(norm_RL,[1 0 0],2);
sdata.RLval=mean_trace';
sdata.RLval_se=se_trace';
