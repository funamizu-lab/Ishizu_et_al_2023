...
----------------------------------------------------------------------------
%}
function  FigureS14c_process_SLR_choice_long_short_tochoice(folders)
...
[mean_trace_s,~,se_trace_s] =plot_mean_se_moto(each_s_choice(2).matrix,[0 0 1],2);
[mean_trace_l,~,se_trace_l] =plot_mean_se_moto(each_l_choice(2).matrix,[1 0 0],2);
...
writetable(T, ['source figS14c before chocie left ',name,'.csv']);


subplot(1,3,2);hold on
[mean_trace_s,~,se_trace_s] =plot_mean_se_moto(each_s_choice(3).matrix,[0 0 1],2);
...
sdata.short_se=se_trace_s';
T = struct2table(sdata);
writetable(T, ['source figS14c before chocie right ',name,'.csv']);


subplot(1,3,3);hold on
[mean_trace_s,~,se_trace_s] =plot_mean_se_moto(each_s_choice(4).matrix,[0 0 1],2);
[mean_trace_l,~,se_trace_l] =plot_mean_se_moto(each_l_choice(4).matrix,[1 0 0],2);
set(gca,'xlim',[0.5 3.5])
set(gca,'ylim',[0.65 1])
%%% source data%%% 
sdata = struct();
...
sdata.short_se=se_trace_s';
T = struct2table(sdata);
writetable(T, ['source figS14c chocie ',name,'.csv']);
